...
queue * init_queue();
int insert_queue(queue * q, node * n, int level);
...
void print_tree(_234t * tree)
{
    ...
    queue * q = init_queue();
    int level = 0;
    insert_queue(q, tree->root, level);

    while(q->size) {
        if (level != q->head->level) {
            printf("\n");
            level = q->head->level;
        }
        node * n = pop_queue(q);
        ...
        switch (n->node) {
            case 1:
                insert_queue(q, n->link1, level + 1);
                insert_queue(q, n->link2, level + 1);
                break;
            case 2:
                insert_queue(q, n->link1, level + 1);
                insert_queue(q, n->link2, level + 1);
                insert_queue(q, n->link3, level + 1);
                break;
            case 3:
                insert_queue(q, n->link1, level + 1);
                insert_queue(q, n->link2, level + 1);
                insert_queue(q, n->link3, level + 1);
                insert_queue(q, n->link4, level + 1);
                break;
            default:
                break;
        }
...
queue * init_queue()
{
    ...
    if (q == NULL) {
        fprintf(stderr, "FATAL ERROR : Heap is full\n");
        exit(-1);
    }
...
{
    if (q == NULL) {
        fprintf(stderr, "ERROR : Failed to insert queue -> q is NULL\n");
        return TRUE;
    }
    if (n == NULL) {
        return TRUE;
...
node * pop_queue(queue * q) {
    ...
    node * result = q->head->data;
    queue_node * dump = q->head;
    q->head = q->head->next;
    q->size--;
    free(dump);
